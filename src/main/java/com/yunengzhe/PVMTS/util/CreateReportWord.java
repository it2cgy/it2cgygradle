...
public class CreateReportWord {
    ...
    static {  
        ...
        configuration.setClassForTemplateLoading(CreateReportWord.class, "/ftl");
        allTemplates = new HashMap<String, Template>();   // Java 7 钻石语法  
        try {  
            allTemplates.put("longieb", configuration.getTemplate("reportDaily.ftl"));  //reportDaily.ftl  test.ftl
        } catch (IOException e) {  
            e.printStackTrace();  
	...
	public static File createDoc(Map<?, ?> dataMap, String Reportname, String type,String downpath) throws IOException { 
		...
		if(downpath==null){
			if (os.startsWith("Windows")) {
				...
				url = "reportsDaily"+"//"+format+"//"+Reportname+ ".doc";
				pathname = path+url;
