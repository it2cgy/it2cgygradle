...
function checkEquipmentNumber(){
	$('#equipmentNumber').blur(function() {
		if($(this).val()!=null&&$(this).val()!=''){
			ynzAjax.post(
					window.ynz.path+"/equipment/checkEquipmentNumber.do",
					{
						'number':$(this).val()
					},
					function(response){ 
...
function fileUpload(row,ID){
	if(row!=null&&row!=''){
		...
		form.ajaxSubmit(function(respense){
			...
			$("#filenamepath"+ID).attr("alt",forma);
			$("#filenamepath"+ID).attr("title",forma);
...
function onSubmit(){
	...
	ynzAjax.post(
			window.ynz.path+"/equipment/addEquipment.do",
			{
			...
			function(response){ 
				message.alert( window.ynz.local.tip,window.ynz.local.action_success,3,null,function(){
					if(response.data==true){
						...
						window.location.href=window.ynz.basePath+"/equipmentPage/equipmentList.do"+param ;
					}
				});
	        },
	        function(e){ 
